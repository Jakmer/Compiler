...
class SymbolTable {
   public:
    SymbolTable()
        : noProcedures(0), address(1) {
          };  // address start from 1 because 0 is reserved for accumulator
   ...
   private:
    ...
    void assignAddress(Symbol &symbol) {
        if(symbol.symbolType == PROCEDURE)
        {
            symbol.address = 0;     // TODO: make it unaddressable - its address is define with assembler operations number
            return;
        }
